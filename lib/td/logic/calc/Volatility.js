...
class Volatility { 
     ...
     */
    let atmStrike = Util.getATMStrike(optionChain).toString();
    ...
    Object.entries(optionChain.callExpDateMap).map(optionDate => {
        ...
        // console.log(strikes)
        Object.entries(optionDateStrikes).map(strike => {
            // let strikeProp = strike[1][0];
            // if (strikeProp.inTheMoney && (strikeProp.daysToExpiration <= daysIVcalc)) {
            //     dateTmp.dte = strikeProp.daysToExpiration;
            //     let vol = strikeProp.volatility;
            //     // if (vol !== 'NaN' && vol !== 5 && vol !== 0) { //cases where volatility = -Inf the API reports with weird values...
            //     //     dateTmp.strikeCount++;
            //     //     dateTmp.sumIV += vol;
            //     //     doCalc = true;
            //     // } 
            ...
            //     }
            // }
            let strikeVal = strike[0];
            if (strikeVal === tickerIVMap.atmStrike) {
                let strikeProp = strike[1][0];
                if (strikeProp.inTheMoney && (strikeProp.daysToExpiration <= daysIVcalc)) {
                    dateTmp.dte = strikeProp.daysToExpiration;
                    let vol = strikeProp.volatility;
